...
//
// $File: //nextgendev/Framework_050/src/FWGLCamControlApplication.cpp $
// $Author: asidwell $
// $Date: 2005/10/10 $
// $Revision: #3 $
//
...
mTurnRate(FWGCMCAMAPP_DEFAULT_TURN_RATE),
mMoveRate(FWGCMCAMAPP_DEFAULT_MOVE_RATE)
{
}
...
// Notes: 
//-----------------------------------------------------------------------------
bool FWGCMCamControlApplication::onInit(int argc, char **ppArgv)
{
	if(FWGCMCamApplication::onInit(argc, ppArgv) == false)
		return false;
...
// Notes: 
//-----------------------------------------------------------------------------
void FWGCMCamControlApplication::onShutdown()
{
	// Call parent class
...
bool FWGCMCamControlApplication::onUpdate()
{
	...
	if(FWGCMCamApplication::onUpdate() == false)
		return false;
...
// Notes: 
//-----------------------------------------------------------------------------
void FWGCMCamControlApplication::onRender()
{
...
// Notes: 
//-----------------------------------------------------------------------------
void FWGCMCamControlApplication::onSize(const FWDisplayInfo &dispInfo)
{
	FWGCMCamApplication::onSize(dispInfo);
}
...
// Notes:
//-----------------------------------------------------------------------------
void FWGCMCamControlApplication::setInputsFlyThru(float deltaTime)
{
...
// Notes:
//-----------------------------------------------------------------------------
void FWGCMCamControlApplication::setInputsLookAt(float deltaTime)
{
	mCamera.setPan(mCamera.getPan() - (mpInputX0 ? mpInputX0->getFloatValue() : 0.f) * deltaTime * mTurnRate);
	mCamera.setTilt(mCamera.getTilt() - (mpInputY0 ? mpInputY0->getFloatValue() : 0.f) * deltaTime * mTurnRate);
